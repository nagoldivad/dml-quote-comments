<?php
/*
Plugin Name: DML Quote Comments
Plugin URI: https://github.com/nagoldivad/dml-quote-comments
...
Version: 0.1
Author: David M. Logan
Author URI: https://github.com/nagoldivad
...
load_plugin_textdomain('dml-quote-comments', NULL, dirname(plugin_basename(__FILE__)) . "/languages");

// Add a define variable, we'll need it later :)
if ( ! defined( 'dml_quote_comments_VERSION' ) ) {
	define( 'dml_quote_comments_VERSION', '2.2' );
}
...
function quote_scripts () {
	...
	wp_register_script('dml_quote_comments_js', ($plugin_url . '/dml-quote-comments.js'), false, '1.0');
	wp_enqueue_script('dml_quote_comments_js');
...
function add_quote_button($output) {
	...
	if (get_option('comment_registration') && !$user_ID) {
		...
		//if (get_option('dml_quote_comments_pluginhook') == "get_comment_time") {
			$button .= '</a>';
		//}
        ...
        // DML
        if ( get_option('dml_quote_comments_classes') ) {
            $button .= '<a class="';
            $button .= get_option('dml_quote_comments_classes');
            $button .= '" href="javascript:void(null)" ';
        } else {
            $button .= '<a class="comment_quote_link" href="javascript:void(null)" ';
        }
        // end DML
        
		$button .= 'title="' . __('Click here or select text to quote comment', 'dml-quote-comments'). '" ';
		
		if( get_option('dml_quote_comments_author') == true ) {
		...
		$button .= 'return false;">';
		$button .= "" . get_option('dml_quote_comments_title') . "";
		
		
		// reply link
		if (get_option('dml_quote_comments_replylink') == true) {
			$button .= '</a>&nbsp;&nbsp;';
			//$button .= '<a class="comment_reply_link btn btn-default btn-sm" href="javascript:void(null)" ';
			...
			$button .= 'title="' . __('Click here to respond to author', 'dml-quote-comments'). '" ';
			...
			$button .= 'try { addComment.moveForm(\'div-comment-'.get_comment_ID().'\', \''.get_comment_ID().'\', \'respond\', \''.get_the_ID().'\'); } catch(e) {}; ';
			$button .= 'return false;">';
			$button .= "" . get_option('dml_quote_comments_replytitle') . "";
		}
		
		
		// close anchor link if body text (if using get comment time, this </a> is already here due to a bug)
		if (get_option('dml_quote_comments_pluginhook') == "get_comment_text") {
			$button .= "</a>";
		}
...
// Options
$qc_themename = "DML Quote Comments";
$qc_shortname = "dml_quote_comments";


$qc_options = array (

	array(	"name" => __('Quote-link title?','dml-quote-comments'),
		//"desc" => __('Title of comment link.','dml-quote-comments'),
		"id" => $qc_shortname."_title",
		"std" => "(Quote)",
		"type" => "text"),

	array(	"name" => __('Show author in quote?','dml-quote-comments'),
		"desc" => __('Show authors','dml-quote-comments'),
		"id" => $qc_shortname."_author",
		"std" => true,
		"type" => "checkbox"),

	array(	"name" => __('Show reply link?','dml-quote-comments'),
		"desc" => __('Show reply link','dml-quote-comments'),
		"id" => $qc_shortname."_replylink",
		"std" => false,
		"type" => "checkbox"),

	array(	"name" => __('Reply-link title?','dml-quote-comments'),
		//"desc" => __('Title of comment link.','dml-quote-comments'),
		"id" => $qc_shortname."_replytitle",
		"std" => "(Reply)",
		"type" => "text"),

	array(	"name" => __('Insert Quote link using which hook?','dml-quote-comments'),
		"desc" => __('Which plugin hook should be used to insert the quote link?','dml-quote-comments'),
    ...
    array(	"name" => __('Custom CSS classes','dml-quote-comments'),
		//"desc" => __('Custom CSS classes.','dml-quote-comments'),
		"id" => $qc_shortname."_classes",
		"std" => "",
		"type" => "text"),
...
function quotecomments_add_admin() {
	...
	if ( ! empty( $_GET['page'] ) && $_GET['page'] == basename(__FILE__) ) {
		...
		if ( ! empty( $_REQUEST['action'] ) && 'save' == $_REQUEST['action'] ) {
			...
			header("Location: options-general.php?page=dml-quote-comments.php&saved=true");
			die;
...
function quotecomments_admin() {
	...
	if (! empty( $_REQUEST['saved'] ) ) {
		echo '<div id="message" class="updated fade"><p><strong>'.$qc_themename.' '.__('settings saved.','dml-quote-comments').'</strong></p></div>';
	}
...
<?php /*if ( function_exists('screen_icon') ) screen_icon(); DEPRECATED */ ?>
<h2><?php echo $qc_themename; _e(': General Options', 'dml-quote-comments'); ?></h2>

<form method="post" action="">

	<p class="submit">
		<input class="button-primary" name="save" type="submit" value="<?php _e('Save changes','dml-quote-comments'); ?>" />    
...
<?php foreach ($qc_options as $value) { 
	...
	switch ( $value['type'] ) {
		...
		?>
		<tr valign="top"> 
			<th scope="row"><label for="<?php echo $value['id']; ?>"><?php echo __($value['name'],'dml-quote-comments'); ?></label></th>
			<td>
				<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_option( $value['id'] ) != "") { echo get_option( $value['id'] ); } else { echo $value['std']; } ?>" />
				<?php 
				if ( ! empty( $value['desc'] ) ) {
					_e($value['desc'],'dml-quote-comments');
				}
				?>
		...
		<tr valign="top"> 
			...
			<td><textarea name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" cols="<?php echo $ta_options['cols']; ?>" rows="<?php echo $ta_options['rows']; ?>"><?php 
				if( get_option($value['id']) != "") {
						echo __(stripslashes(get_option($value['id'])),'dml-quote-comments');
					}else{
						echo __($value['std'],'dml-quote-comments');
				}?></textarea><br /><?php echo __($value['desc'],'dml-quote-comments'); ?></td>
		</tr>
		<?php
		break;
		...
		?>
		<tr valign="top"> 
			<th scope="row"><?php echo __($value['name'],'dml-quote-comments'); ?></th>
			<td>
		...
		<tr valign="top"> 
			...
			<td>
				...
				<label for="<?php echo $value['id']; ?>"><?php echo __($value['desc'],'dml-quote-comments'); ?></label>
			</td>
		</tr>
		<?php
		break;
